...
public class ShowActivity extends AppCompatActivity {
    ...
    boolean iskey=false;
    String deckey = "AABB09182736CCDD";
    ...
    String binToHex(String input)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            input = Long.toHexString(parseUnsignedLong(input, 2));
        }
        while (input.length() < n)
            input = "0" + input;
        return input;
    ...
    String xor(String a, String b)
    {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            t_a = Long.parseUnsignedLong(a, 16);
        }
        // hexadecimal to decimal(base 10)
        Long t_b = null;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            t_b = Long.parseUnsignedLong(b, 16);
        }
        // xor
        t_a = t_a ^ t_b;
   ...
   public String removezeros(String input)
   {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String keyT = intent.getStringExtra("key");
        String keyen = encryptusinghash(keyT);
        list = new ArrayList<>();
   ...
   private void fetchdata(String keyen)
   {
       db.collection("usersdetails").get()
               .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                   ...
                   public void onComplete(@NonNull Task<QuerySnapshot> task) {
                       ...
                       {
                           String k = doc.getString("keyText");
                           if(k.equalsIgnoreCase(keyen))
                           {
                               db.collection("stockslist").get()
                                       .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                                           @Override
                                           public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                               list.clear();
                                               for(QueryDocumentSnapshot doc: task.getResult())
                                               {
                                                   ...
                                                   String customerprice = doc.getString("customerprice");
                                                   String wholesaleprice = doc.getString("wholesaleprice");
                                                   String totalstocks = doc.getString("totalstocks");
                                                   String decwholesaleprice = decrypt(wholesaleprice,deckey);
                                                   String dectotalstocks = decrypt(totalstocks,deckey);
                                                   String dwholesaleprice = removezeros(decwholesaleprice);
                                                   String dtotalstocks = removezeros(dectotalstocks);

                                                   Model model = new Model("id "+id,"product name "+productname,"customer price "+customerprice,"wholesale price "+dwholesaleprice,"total stocks "+dtotalstocks);
                                                   list.add(model);
                                               }
                                   ...
                                   public void onFailure(@NonNull  Exception e) {
                                       Toast.makeText(ShowActivity.this, "Failed to get !!", Toast.LENGTH_SHORT).show();
                                   }
                               });
                           }
                           else
                           {
                               Toast.makeText(ShowActivity.this, "invalid key !!", Toast.LENGTH_SHORT).show();
